...
test('HashTable.data.length should be the same length as the size passed into constructor', () => {
  for (let i = 0; i < 5; i++) {
    const SIZE = i * 10;
...
test('HashTable.data elements should be undefined when instantiated', () => {
  const SIZE = 10;
  const hashTable = new HashTable(SIZE);
...
test('HashTable._hash should return number from zero to (size-1)', () => {
  const SIZE = 10;
  const hashTable = new HashTable(SIZE);
...
test('HashTable.set(key, value) should append [key, value] to data[hash]', () => {
  const SIZE = 10;
  const hashTable = new HashTable(SIZE);
...
test('HashTable.get(key) should return the set value', () => {
  const SIZE = 10;
  const hashTable = new HashTable(SIZE);
  const KEY = 'apples';
  const VALUE = 15;
