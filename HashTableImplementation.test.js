...
test('1. HashTable.data elements should be undefined when instantiated', () => {
  const SIZE = 10;
  const hashTable = new HashTable(SIZE);
...
test('2. HashTable._hash should return number from zero to (size-1)', () => {
  const SIZE = 10;
  const hashTable = new HashTable(SIZE);
...
test('3. HashTable.set(key, value) should append [key, value] to data[hash]', () => {
  const SIZE = 10;
  const hashTable = new HashTable(SIZE);
...
test('4. HashTable.get(key) should return the set value', () => {
  const SIZE = 10;
  const hashTable = new HashTable(SIZE);
  const KEY = 'apples';
  const VALUE = 15;
